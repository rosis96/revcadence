...
class MailboxConnection(Base):
    ...
    # Workspace-level follow-up autopilot defaults (new deals inherit these)
    default_autopilot = Column(Boolean, default=False)
    default_interval_days = Column(Integer, default=4)
    default_max_followups = Column(Integer, default=4)
    created_at = Column(DateTime, default=datetime.utcnow)
    updated_at = Column(DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)
