...
def build_ai_cfg(rws) -> dict:
    ...
    return {
        ...
        # Model for BOTH reading (intent) and writing (reply). The workspace's own
        # choice first — same precedence as the keys above, and the reason Reply
        # Settings is a per-workspace screen rather than an org one. Then a
        # reply-specific env override, then the shared OPENAI_MODEL set in Railway
        # (this is the one the deploy actually has), then a strong default.
        # gpt-5-mini was silently the default and isn't available on this account,
        # which stalled every draft.
        "openai_model": ((rws.openai_model or "").strip() or os.getenv("REPLY_OPENAI_MODEL")
                         or os.getenv("OPENAI_MODEL") or "gpt-4o"),
        "gemini_model": ((rws.gemini_model or "").strip() or os.getenv("REPLY_GEMINI_MODEL")
                         or os.getenv("GEMINI_MODEL") or "gemini-2.5-pro"),
    }
